...
class AjaxProductPriceController extends AbstractAjaxProductPriceController
{
   ...
   //     var_dump($request->query);exit;
        ...
        $productType = 1;
        $productYear = date("Y");       
        $apiUrl = $this->getParameter("aakrib_api_get_netprice");
        $apiKey = $this->getParameter("aakrib_api_access_key");        
        //var_dump($productIds);exit;
        if($productIds)
        {
            foreach($productIds as $productId){
                $productSku=$this->getDoctrine()->getEntityManager()->find(Product::class, $productId)->getSku();
                ...
                $responseData = $this->get('api_caller')->call(new HttpGetJson($apiUrl,$requestArray));
                
                $json  = json_encode($responseData);
                $returnData[$productId] = json_decode($json, true);
