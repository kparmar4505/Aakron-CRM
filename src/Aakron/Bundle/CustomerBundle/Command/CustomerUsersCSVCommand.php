...
class CustomerUsersCSVCommand extends ContainerAwareCommand implements CronCommandInterface
{
        ...
        if (($fp = fopen($sourceFile, "r")) !== FALSE) {
            while (($rows = fgetcsv($fp, 10000, ",")) !== FALSE) {
                ...
                if($rowNo>0){
                    ...
                    $responseArray["customer"][$accountNumber]["users"][$rowNo]["title"] = $rows[8]??"";
                    ...
                    $responseArray["customer"][$accountNumber]["users"][$rowNo]['street']=$rows[9]??"";
                    $responseArray["customer"][$accountNumber]["users"][$rowNo]['street2']=$rows[10]??"";
    ...
    public function addCustomerAddress($customerData,$customerId)
    {
        ...
        $postUrl = $this->getContainer()->getParameter("customer_address.destination.url");
        
        $customerAddressArray['data']['type']="customer_address";
        $customerAddressArray['data']['attributes']['city']=$customerData['city']??"";
        $customerAddressArray['data']['attributes']['firstName']="";
        $customerAddressArray['data']['attributes']['label']="Primary address";
        $customerAddressArray['data']['attributes']['lastName']=$customerData['lastName']??"";
        $customerAddressArray['data']['attributes']['middleName']=$customerData['middleName']??"";
        $customerAddressArray['data']['attributes']['namePrefix']="";
        $customerAddressArray['data']['attributes']['nameSuffix']=$customerData['nameSuffix']??"";
        $customerAddressArray['data']['attributes']['organization']=$customerData['accountName']??"";
        $customerAddressArray['data']['attributes']['phone']=$customerData['phones']??"";
        $customerAddressArray['data']['attributes']['postalCode']=$customerData['postalCode']??"";
        $customerAddressArray['data']['attributes']['primary']=true;
        $customerAddressArray['data']['attributes']['street']=$customerData['street']??"";
        $customerAddressArray['data']['attributes']['street2']=$customerData['street2']??"";
        
        
        $customerAddressArray['data']['relationships']['country']['data']=array('type' => 'countries','id' => 'US');
        $customerAddressArray['data']['relationships']['frontendOwner']['data']=array('type' => 'customers','id' => $customerId);
        $customerAddressArray['data']['relationships']['owner']['data']=array('type' => 'users','id' => '1');
        $customerAddressArray['data']['relationships']['region']['data']=array('type' => 'regions','id' => $customerData['region']??"");
        
        
        //  print_r($customerAddressArray);exit;
        
        $responseAddressData = $apiCallerManager->call(new HttpPostJsonBody($postUrl, $customerAddressArray, false, $options));
        //   var_dump($responseAddressData);exit;
    }
    public function addCustomerUser($customerData,$customerId)
    {
        ...
        $this->addCustomerAddress($customerData,$customerId);
