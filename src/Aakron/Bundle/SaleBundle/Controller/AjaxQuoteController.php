...
class AjaxQuoteController extends Controller
{
     ...
     * Get order related data
     *
     * @Route("/aakron-related-data", options={"expose"=true}, name="aakron_quote_related_data")
     * @Method({"GET"})
     * @AclAncestor("oro_quote_update")
     *
     * @return JsonResponse
     */
    public function getAakronRelatedDataAction()
    {
        $quote = new Quote();
       // dump($this->getQuoteRequestHandler());exit;
        $customerUser = $this->getCustomQuoteRequestHandler()->getCustomerUser();
        
        $customer = $this->getAccount($customerUser);
 //       dump($customer);exit;
        $quote->setAccounts($customer);
        $quote->setAccountContacts($customerUser);
    ...
    protected function getCustomQuoteRequestHandler()
    {
        return $this->get('aakron_sale.service.quote_request_handler');
    }
    /**
     * @param CustomerUser $customerUser
     * @return null|Customer
     */
    protected function getAccount(Contact $customerUser = null)
    {
        $customer = $this->getCustomQuoteRequestHandler()->getCustomer();
        
   //     dump($customerUser->getAccounts());
    //    exit;
        if (!$customer && $customerUser) {
            $customer = $customerUser->getAccounts();
        }
        if ($customer && $customerUser) {
            $this->validateRelationCustom($customerUser, $customer);
        }
     ...
     * @throws BadRequestHttpException
     */
    protected function validateRelationCustom(Contact $customerUser, Account $customer)
    {
     //   dump($customerUser->getAccounts());exit;
        if ($customerUser &&
            $customerUser->getAccounts() &&
            $customerUser->getAccounts()->getId() !== $customer->getId()
            ) {
                throw new BadRequestHttpException('CustomerUser must belong to Customer');
            }
    }
